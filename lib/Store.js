...
merge(Store.prototype, EventEmitter.prototype, {
  ...
  delete: function(namespace, model, options) {
    ...
    return this._syncify(found, options);
  },

  reload: function(namespace, model, options) {
    var bucket = this._bucket(namespace);
    var found  = bucket[model.id];

    this.adapter.read(found);
    this.emit(Store.RELOAD_EVENT, found);

    return this._syncify(found, options);
  },

  save: function(namespace, model, options) {
    var bucket = this._bucket(namespace);
    var found  = bucket[model.id];

    this.adapter.update(found);
    this.emit(Store.CHANGE_EVENT, found);

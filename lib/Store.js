...
merge(Store.prototype, Events, {
  ...
  parse: function(payload, options) {
    ...
    Object.keys(payload).forEach(function(key) {
      ...
      models = values.reduce(function(memo, value) {
        memo = memo.concat(self.add(namespace, value));

        return memo;
      }, []);

      if (options && options.action) {
        emitEvent(self, namespace, options.action, models);
      }
    });
  ...
  destroy: function(model) {
    ...
    emitEvent(this, namespace, ACTIONS.destroy, model);
  ...
  save: function(model) {
    var method = model.id ? ACTIONS.update : ACTIONS.create,
        self = this;
